//
// "$Id: TextBuffer.h 9367 2012-04-21 15:16:27Z fabien $"
//
...
/* \file
 fltk3::TextBuffer, fltk3::TextSelection widget . */
...
/**
 \class fltk3::TextSelection
 \brief This is an internal class for fltk3::TextBuffer to manage text selections.
 This class works correctly with utf-8 strings assuming that the parameters
 for all calls are on character boundaries.
 */
class FLTK3_EXPORT TextSelection
{
...
typedef void (*TextPredeleteCb)(int pos, int nDeleted, void* cbArg);
...
/**
 ...
 The fltk3::TextBuffer class is used by the fltk3::TextDisplay
 and fltk3::TextEditor to manage complex text data and is based upon the
 excellent NEdit text editor engine - see http://www.nedit.org/.
 */
class FLTK3_EXPORT TextBuffer
{
public:
	 ...
	 */
	TextBuffer(int requestedSize = 0, int preferredGapSize = 1024);
	 ...
	 */
	void copy(fltk3::TextBuffer* fromBuf, int fromStart, int fromEnd, int toPos);
	...
	/**
	 ...
	 If the input file is not UTF-8-encoded, the fltk3::TextBuffer widget will contain
	 UTF-8-transcoded data. By default, the message fltk3::TextBuffer::file_encoding_warning_message
	...
	/**
	 ...
	 \code
	 typedef void (*fltk3::TextModifyCb)(int pos, int nInserted, int nDeleted,
	 int nRestyled, const char* deletedText,
	 void* cbArg);
	 \endcode
	 */
	void add_modify_callback(fltk3::TextModifyCb bufModifiedCB, void* cbArg);

	/**
	 Removes a modify callback.
	 */
	void remove_modify_callback(fltk3::TextModifyCb bufModifiedCB, void* cbArg);
	 ...
	 */
	void add_predelete_callback(fltk3::TextPredeleteCb bufPredelCB, void* cbArg);
	 ...
	 */
	void remove_predelete_callback(fltk3::TextPredeleteCb predelCB, void* cbArg);
	 ...
	 */
	const fltk3::TextSelection* primary_selection() const {
		return &mPrimary;
	}
	...
	/**
	 Returns the primary selection.
	 */
	fltk3::TextSelection* primary_selection() {
		return &mPrimary;
	}
	 ...
	 */
	const fltk3::TextSelection* secondary_selection() const {
		return &mSecondary;
	}
	 ...
	 */
	const fltk3::TextSelection* highlight_selection() const {
		return &mHighlight;
	}
	 ...
	 */
	void (*transcoding_warning_action)(fltk3::TextBuffer*);
...
protected:
	 ...
	 */
	void redisplay_selection(fltk3::TextSelection* oldSelection,
	                         fltk3::TextSelection* newSelection) const;
	...
	char* selection_text_(fltk3::TextSelection* sel) const;

	/**
	 Removes the text from the buffer corresponding to \p sel.
	 */
	void remove_selection_(fltk3::TextSelection* sel);

	/**
	 Replaces the \p text in selection \p sel.
	 */
	void replace_selection_(fltk3::TextSelection* sel, const char* text);
	...
	fltk3::TextSelection mPrimary;     /**< highlighted areas */
	fltk3::TextSelection mSecondary;   /**< highlighted areas */
	fltk3::TextSelection mHighlight;   /**< highlighted areas */
	...
	fltk3::TextModifyCb *mModifyProcs;/**< procedures to call when buffer is
                                     modified to redisplay contents */
	void** mCbArgs;                 /**< caller arguments for modifyProcs above */
	int mNPredeleteProcs;           /**< number of pre-delete procs attached */
	fltk3::TextPredeleteCb *mPredeleteProcs; /**< procedure to call before text is deleted
...
//
// End of "$Id: TextBuffer.h 9367 2012-04-21 15:16:27Z fabien $".
//
