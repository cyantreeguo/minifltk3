//
// "$Id: Pixmap.h 9422 2012-05-01 14:52:38Z manolo $"
//
...
public:
	...
	explicit Pixmap(char * const * D) : fltk3::Image(-1,0,1), alloc_data(0), id_(0), mask_(0) {
		set_data((const char*const*)D);
		measure();
	}
	/**    The constructors create a new pixmap from the specified XPM data.  */
	explicit Pixmap(uchar* const * D) : fltk3::Image(-1,0,1), alloc_data(0), id_(0), mask_(0) {
		set_data((const char*const*)D);
		measure();
	}
	/**    The constructors create a new pixmap from the specified XPM data.  */
	explicit Pixmap(const char * const * D) : fltk3::Image(-1,0,1), alloc_data(0), id_(0), mask_(0) {
		set_data((const char*const*)D);
		measure();
	}
	/**    The constructors create a new pixmap from the specified XPM data.  */
	explicit Pixmap(const uchar* const * D) : fltk3::Image(-1,0,1), alloc_data(0), id_(0), mask_(0) {
		set_data((const char*const*)D);
		measure();
	}
	virtual ~Pixmap();
...
//
// End of "$Id: Pixmap.h 9422 2012-05-01 14:52:38Z manolo $".
//
