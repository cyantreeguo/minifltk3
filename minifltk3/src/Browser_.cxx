//
// "$Id: Browser_.cxx 9651 2012-08-03 11:29:01Z matt $"
//
...
{
	if (!redraw1 || redraw1 == item) {
		redraw1 = item;
		damage(fltk3::DAMAGE_EXPOSE);
	} else if (!redraw2 || redraw2 == item) {
		redraw2 = item;
		damage(fltk3::DAMAGE_EXPOSE);
	...
	if (position_ != real_position_) {
		...
		if (!l) {
			...
			for (;;) {
				...
				l = l1;
				yy = position_ = ly = ly-item_quick_height(l);
			}
			// use it:
			top_ = l;
			offset_ = yy-ly;
...
void fltk3::Browser_::display(void* item)
{
	...
	if (l == item) {
		position(real_position_+Y);        // scroll up a bit
		return;
	}
	...
	void* lp = item_prev(l);
	if (lp == item) {
		position(real_position_+Y-item_quick_height(lp));
		return;
	}
...
J1:
	...
	int scrollsize = scrollbar_size_ ? scrollbar_size_ : fltk3::scrollbar_size();
	int dy = top_ ? item_quick_height(top_) : 0;
	if (dy < 10) dy = 10;
	if (scrollbar.visible()) {
...
TOGGLE:
				...
				if (!l) down = 1;
					...
					for (void* m = selection_; ; m = item_next(m)) {
						if (m == l) {
							down = 1;
							break;
						}
	...
	case fltk3::DRAG:
		...
		if (type() == fltk3::NORMAL_BROWSER || !top_)
			...
			void* l = find_item(my);
			void* t;
			void* b; // this will be the range to change
			if (my > py) { // go down
...
//
// End of "$Id: Browser_.cxx 9651 2012-08-03 11:29:01Z matt $".
//
