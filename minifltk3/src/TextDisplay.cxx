//
// "$Id: TextDisplay.cxx 10152 2014-05-21 06:56:59Z greg.ercolano $"
//
 ...
 */
fltk3::TextDisplay::TextDisplay(int X, int Y, int W, int H, const char* l) : fltk3::Group(X, Y, W, H, l)
{
	int i;
	...
	mCursorStyle = SIMPLE_CURSOR; // NORMAL_CURSOR;
	...
	linenumber_font_    = fltk3::HELVETICA;
	linenumber_size_    = fltk3::NORMAL_SIZE;
	linenumber_fgcolor_ = fltk3::INACTIVE_COLOR;
	linenumber_bgcolor_ = 53;	// ~90% gray
	linenumber_align_   = fltk3::ALIGN_RIGHT;
...
*/
void fltk3::TextDisplay::linenumber_width(int width)
{
	if (width < 0) return;
	mLineNumWidth = width;
...
*/
int fltk3::TextDisplay::linenumber_width() const
{
...
*/
void fltk3::TextDisplay::linenumber_font(fltk3::Font val)
{
#if FLTK_ABI_VERSION >= 10303
...
*/
fltk3::Font fltk3::TextDisplay::linenumber_font() const
{
#if FLTK_ABI_VERSION >= 10303
	return linenumber_font_;
#else
	return fltk3::HELVETICA;
#endif
}
...
/**
 ...
 \version 1.3.3 ABI feature (ignored in 1.3.x unless FLTK_ABI_VERSION is 10303 or higher)
*/
void fltk3::TextDisplay::linenumber_size(fltk3::Fontsize val)
{
#if FLTK_ABI_VERSION >= 10303
...
*/
fltk3::Fontsize fltk3::TextDisplay::linenumber_size() const
{
#if FLTK_ABI_VERSION >= 10303
	return linenumber_size_;
#else
	return fltk3::NORMAL_SIZE;
#endif
}
...
/**
 ...
 \version 1.3.3 ABI feature (ignored in 1.3.x unless FLTK_ABI_VERSION is 10303 or higher)
*/
void fltk3::TextDisplay::linenumber_fgcolor(fltk3::Color val)
{
#if FLTK_ABI_VERSION >= 10303
...
*/
fltk3::Color fltk3::TextDisplay::linenumber_fgcolor() const
{
#if FLTK_ABI_VERSION >= 10303
	return linenumber_fgcolor_;
#else
	return fltk3::INACTIVE_COLOR;
#endif
}
...
/**
 ...
 \version 1.3.3 ABI feature (ignored in 1.3.x unless FLTK_ABI_VERSION is 10303 or higher)
*/
void fltk3::TextDisplay::linenumber_bgcolor(fltk3::Color val)
{
#if FLTK_ABI_VERSION >= 10303
...
*/
fltk3::Color fltk3::TextDisplay::linenumber_bgcolor() const
{
#if FLTK_ABI_VERSION >= 10303
...
/**
 ...
 Valid values are fltk3::ALIGN_LEFT, fltk3::ALIGN_CENTER or fltk3::ALIGN_RIGHT.
 \version 1.3.3 ABI feature (ignored in 1.3.x unless FLTK_ABI_VERSION is 10303 or higher)
*/
void fltk3::TextDisplay::linenumber_align(fltk3::Align val)
{
#if FLTK_ABI_VERSION >= 10303
...
*/
fltk3::Align fltk3::TextDisplay::linenumber_align() const
{
#if FLTK_ABI_VERSION >= 10303
	return linenumber_align_;
#else
	return fltk3::ALIGN_RIGHT;
#endif
}
...
/**
 ...
 \version 1.3.3 ABI feature (ignored in 1.3.x unless FLTK_ABI_VERSION is 10303 or higher)
*/
void fltk3::TextDisplay::linenumber_format(const char* val)
{
#if FLTK_ABI_VERSION >= 10303
...
*/
const char* fltk3::TextDisplay::linenumber_format() const
{
#if FLTK_ABI_VERSION >= 10303
...
void fltk3::TextDisplay::draw_range(int startpos, int endpos)
{
	...
	if ( startpos < mFirstChar ) startpos = mFirstChar;
	if ( !position_to_line( startpos, &startLine ) ) startLine = mNVisibleLines - 1;
	if ( endpos >= mLastChar ) {
	...
	if ( endpos >= mLastChar ) endIndex = INT_MAX;
	else if ( mLineStarts[ lastLine ] == -1 ) endIndex = 0;
	else endIndex = endpos - mLineStarts[ lastLine ];
	...
	for ( i = startLine + 1; i < lastLine; i++ ) draw_vline( i, 0, INT_MAX, 0, INT_MAX );
...
/**
 ...
 \return 1 if position (X, Y) is inside of the primary fltk3::Text_Selection
 */
                                     ...
                                     const char *string, int nChars) const
{
	...
	/* Set font, color, and gc depending on style.  For normal text, GCs
	 for normal drawing, or drawing within a fltk3::Text_Selection or highlight are
	...
	/*
	 ...
	 mDescent - fs->descent);
	 */
	/* Underline if style is secondary fltk3::Text_Selection */
...
void fltk3::TextDisplay::draw_line_numbers(bool /*clearAll*/)
{
	...
	int xoff = fltk3::box_dx(box());
	int hscroll_h = mHScrollBar->visible() ? mHScrollBar->h() : 0;
	//fltk3::push_clip(x()+xoff, y()+fltk3::box_dy(box()), mLineNumWidth - xoff, h() - fltk3::box_dh(box()) - hscroll_h);
	fltk3::push_clip(xoff, fltk3::box_dy(box()), mLineNumWidth - xoff, h() - fltk3::box_dh(box()) - hscroll_h);
	{
		// Set background color for line number area -- LZA / STR# 2621
		// Erase background
		fltk3::color(linenumber_bgcolor());
		//fltk3::rectf(x(), y(), mLineNumWidth, h());
		fltk3::rectf(0, 0, mLineNumWidth, h());

		// Draw separator line
		//fltk3::color(180,180,180);
		//fltk3::line(x()+mLineNumWidth-1, y(), x()+mLineNumWidth-1, y()+h());

		// Draw line number text
		fltk3::font(linenumber_font(), linenumber_size());
		...
		fltk3::color(linenumber_fgcolor());
		for (visLine=0; visLine < mNVisibleLines; visLine++) {
			...
			if (lineStart != -1 && (lineStart==0 || buffer()->char_at(lineStart-1)=='\n')) {
				...
				int xx = 0 + xoff + 3,
				    ...
				    hh = lineHeight;
				fltk3::draw(lineNumString, xx, yy, ww, hh, linenumber_align(), 0, 0);
				//DEBUG fltk3::rect(xx, yy, ww, hh);
				//last_y = Y;
				line++;
			} else {
				if (visLine == 0) line++;
...
void fltk3::TextDisplay::extend_range_for_styles( int *startpos, int *endpos )
{
	...
	/* The peculiar protocol used here is that modifications to the style
	 buffer are marked by selecting them with the buffer's primary fltk3::Text_Selection.
	...
	/* If the fltk3::Text_Selection was extended due to a style change, and some of the
...
void fltk3::TextDisplay::draw(void)
{
	...
	if (damage() & fltk3::DAMAGE_ALL) {
		...
		if (mHScrollBar->visible()) W -= scrollbar_width();
		if (mVScrollBar->visible()) H -= scrollbar_width();

		// left margin
		fltk3::rectf(text_area.x-LEFT_MARGIN, text_area.y-TOP_MARGIN, LEFT_MARGIN, text_area.h+TOP_MARGIN+BOTTOM_MARGIN, color());

		// right margin
		fltk3::rectf(text_area.x+text_area.w, text_area.y-TOP_MARGIN, RIGHT_MARGIN, text_area.h+TOP_MARGIN+BOTTOM_MARGIN, color());

		// top margin
		fltk3::rectf(text_area.x, text_area.y-TOP_MARGIN, text_area.w, TOP_MARGIN, color());

		// bottom margin
		fltk3::rectf(text_area.x, text_area.y+text_area.h, text_area.w, BOTTOM_MARGIN, color());

		// draw that little box in the corner of the scrollbars
		if (mVScrollBar->visible() && mHScrollBar->visible()) fltk3::rectf(mVScrollBar->x(), mHScrollBar->y(), mVScrollBar->w(), mHScrollBar->h(), fltk3::GRAY);
		...
		fltk3::push_clip(text_area.x-LEFT_MARGIN, text_area.y, text_area.w+LEFT_MARGIN+RIGHT_MARGIN, text_area.h);
		fltk3::rectf(text_area.x-LEFT_MARGIN, mCursorOldY, LEFT_MARGIN, mMaxsize, color());
		fltk3::rectf(text_area.x+text_area.w, mCursorOldY, RIGHT_MARGIN, mMaxsize, color());
		fltk3::pop_clip();
	}
	...
	if (damage() & (fltk3::DAMAGE_ALL | fltk3::DAMAGE_EXPOSE)) {
		...
		int X, Y, W, H;
		if (fltk3::clip_box(text_area.x, text_area.y, text_area.w, text_area.h, X, Y, W, H)) {
		...
		fltk3::push_clip(text_area.x, text_area.y, text_area.w, text_area.h);
...
int fltk3::TextDisplay::handle(int event)
{
	...
	case fltk3::PUSH: {
		...
		if (dragType == DRAG_CHAR) {
			buffer()->unselect();
//	fltk3::copy("", 0, 0); /* removed for STR 2668 */
...
//
// End of "$Id: fltk3::TextDisplay.cxx 10152 2014-05-21 06:56:59Z greg.ercolano $".
//
