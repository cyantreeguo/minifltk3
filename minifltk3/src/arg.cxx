//
// "$Id: arg.cxx 9376 2012-04-22 13:09:19Z matt $"
//
...
int fltk3::arg(int argc, char **argv, int &i)
{
	...
	if (!s) {
		i++;        // something removed by calling program?
		return 1;
	}
...
void fltk3::Window::show(int argc, char **argv)
{
	...
	int j;
	int n=0;
	for (j=0; j<argc; j++) n += strlen(argv[j])+1;
...
static int argc_;
static char **argv_;
void fltk3::arg_param(int *argc, char **argv[])
{
	*argc = argc_;
	*argv = argv_;
}
...
void fltk3::args(int argc, char **argv)
{
	...
	if (fltk3::args(argc,argv,i) < argc) fltk3::error(helpmsg);
}
...
//
// End of "$Id: arg.cxx 9376 2012-04-22 13:09:19Z matt $".
//
