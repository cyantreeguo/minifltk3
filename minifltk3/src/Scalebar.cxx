	...
	if (type() == fltk3::VERT_NICE_SLIDER) {
		...
		if ( horizontal() ) {
			if ( hsl>6 && wsl>=hsl*2.5f ) {
				draw_box( pushed_==4 ? box1->downbox() : box1, xsl+2, ysl+2, hsl-4, hsl-4, selection_color() );
				draw_box( pushed_==5 ? box1->downbox() : box1, xsl+wsl-hsl+2, ysl+2, hsl-4, hsl-4, selection_color() );
			}
		} else {
			if ( wsl>6 && hsl>=wsl*2.5f ) {
				draw_box( pushed_==4 ? box1->downbox() : box1, xsl+2, ysl+2, wsl-4, wsl-4, selection_color() );
				draw_box( pushed_==5 ? box1->downbox() : box1, xsl+2, ysl+hsl-wsl+2, wsl-4, wsl-4, selection_color() );
			}
		}
	}
...
void fltk3::Scalebar::draw()
{
	...
	if (horizontal()) {
		...
		if (damage()&fltk3::DAMAGE_ALL) {
			// left arrow box
			draw_box((pushed_==1) ? slider()->downbox() : slider(), X, Y, H, H, selection_color());
			// right arrow box
			draw_box((pushed_==2) ? slider()->downbox() : slider(), X+W-H, Y, H, H, selection_color());
		...
		if (damage()&fltk3::DAMAGE_ALL) {
			draw_box((pushed_==1) ? slider()->downbox() : slider(), X, Y, W, W, selection_color());
			draw_box((pushed_==2) ? slider()->downbox() : slider(), X, Y+H-W, W, W, selection_color());
			if (active_r())
				fltk3::color(labelcolor());
			else
				fltk3::color(fltk3::inactive(labelcolor()));
