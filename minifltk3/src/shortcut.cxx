//
// "$Id: shortcut.cxx 9011 2011-08-25 21:50:59Z matt $"
//
...
const char* fltk3::shortcut_label(unsigned int shortcut, const char **eom)
{
	...
	if (shortcut & fltk3::SHIFT) {
		strcpy(p,"\xe2\x87\xa7");        // upwards white arrow
		p += 3;
	}
	if (shortcut & fltk3::CTRL)  {
		strcpy(p,"\xe2\x8c\x83");        // up arrowhead
		p += 3;
	}
	if (shortcut & fltk3::ALT)   {
		strcpy(p,"\xe2\x8c\xa5");        // alternative key symbol
		p += 3;
	}
	if (shortcut & fltk3::META)  {
		strcpy(p,"\xe2\x8c\x98");        // place of interest sign
		p += 3;
	}
#else
	if (shortcut & fltk3::META) {
...
//
// End of "$Id: shortcut.cxx 9011 2011-08-25 21:50:59Z matt $".
//
