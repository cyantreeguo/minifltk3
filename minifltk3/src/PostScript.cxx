//
// "$Id: PostScript.cxx 9951 2013-07-16 19:25:44Z manolo $"
//
...
#  include "Platform_x11_print_panel.cxx"
#endif
...
//returns 0 iff OK
{
	...
	if (lang_level_ == 3 && (layout & fltk3::PagedDevice::LANDSCAPE) ) {
		x = w;
		w = h;
		h = x;
	}
...
int fltk3::PostScriptPrinter::start_job(int pages, int *firstpage, int *lastpage)
{
	...
	{
		char tmp[10];
		snprintf(tmp, sizeof(tmp), "%d", pages);
		print_to->value(tmp);
	}
...
//
// End of "$Id: PostScript.cxx 9951 2013-07-16 19:25:44Z manolo $".
//
