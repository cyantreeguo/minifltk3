//
// "$Id: draw.cxx 9330 2012-04-09 08:53:49Z manolo $"
//
...
*/
const char* fltk3::expand_text(const char* from, char* buf, int maxbuf, double maxw, int& n, double &width, int wrap, int draw_symbols)
{
	char* o = buf;
	char* e = buf+(maxbuf-4);
	underline_at = 0;
	...
	//clock_t ct = clock(); int skew;//printf("expand_text start %d, %s========\n", clock(), from);

	const char* p = from;
	for (;; p++) {

		//skew = (int)(clock()-ct); printf("[%d] ", skew);
	...
	//printf("\nexpand_text stop  [%d] %d, %s\n", int(clock()-ct), clock(), from);
        ...
        fltk3::Image* img, int draw_symbols)
{
	...
	if (str) {
		// clock_t ct = clock(); printf("draw_label start %d, %s========\n", clock(), str);
		for (p = str, lines=0; p;) {
		...
		// printf("draw_label stop  %d[%d], %s\n", clock(), int(clock()-ct), str);
	} else {
		lines = 0;
	}
...
//
// End of "$Id: draw.cxx 9330 2012-04-09 08:53:49Z manolo $".
//
