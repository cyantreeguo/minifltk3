//
// "$Id: Menu.cxx 9739 2012-12-07 16:33:22Z AlbrechtS $"
//
...
BACKTAB:
	...
	case fltk3::SHORTCUT: {
		for (int mymenu = pp.nummenus; mymenu--;) {
			...
			int item;
			const fltk3::MenuItem* m = mw.menu->find_shortcut(&item);
			if (m) {
        ...
        int menubar) const
{
	...
	for (;;) {
		...
		if(pp.fakemenu) {
			delete pp.fakemenu;        // turn off "menubar button"
			pp.fakemenu = 0;
		}
...
//
// End of "$Id: Menu.cxx 9739 2012-12-07 16:33:22Z AlbrechtS $".
//
