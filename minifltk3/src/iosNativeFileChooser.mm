// "$Id: cocoaNativeFileChooser.mm 9828 2013-02-25 17:29:54Z manolo $"
//
...
void fltk3::NativeFileChooser::parse_filter(const char *in) {
	...
	for (; 1; in++) {
		...
		regchar:              // handle regular char
				switch (mode) {
					case 'n':
						chrcat(name, *in);     continue;
					case 'w':
						chrcat(wildcard, *in); continue;
				}
				break;
		}
	}
...
int fltk3::NativeFileChooser::post() {
	...
	switch (_btype) {
		...
		case BROWSE_MULTI_DIRECTORY:
			...
			/* FALLTHROUGH */
		case BROWSE_DIRECTORY:
			[(NSOpenPanel *)_panel setCanChooseDirectories:
			 YES];
			break;
		case BROWSE_SAVE_DIRECTORY:
			[(NSSavePanel *)_panel setCanCreateDirectories:
			 YES];
			break;
	}
...
//
// End of "$Id: cocoaNativeFileChooser.mm 9828 2013-02-25 17:29:54Z manolo $".
//
