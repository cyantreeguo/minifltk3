//
// "$Id: Table.cxx 9479 2012-05-10 22:06:06Z matt $"
//
		...
		if ( fltk3::event_inside(X, Y, W, H) ) {
			...
			for ( R = toprow; R <= botrow; R++ ) {
				...
				if ( fltk3::event_y() >= Y && fltk3::event_y() < (Y+H) ) {
					...
					//
					if ( row_resize() ) {
						if ( fltk3::event_y() <= (Y+3-0) ) {
							resizeflag = RESIZE_ROW_ABOVE;
						}
						if ( fltk3::event_y() >= (Y+H-3) ) {
		...
		if ( fltk3::event_inside(X, Y, W, H) ) {
			...
			for ( C = leftcol; C <= rightcol; C++ ) {
				...
				if ( fltk3::event_x() >= X && fltk3::event_x() < (X+W) ) {
					...
					//
					if ( col_resize() ) {
						if ( fltk3::event_x() <= (X+3-0) ) {
							resizeflag = RESIZE_COL_LEFT;
						}
						if ( fltk3::event_x() >= (X+W-3) ) {
...
void fltk3::Table::recalc_dimensions()
{
	...
	wix = (       fltk3::box_dx(box()));
	tox = wix;
	tix = tox + fltk3::box_dx(table->box());
	wiy = (       fltk3::box_dy(box()));
	toy = wiy;
	tiy = toy + fltk3::box_dy(table->box());
	wiw = ( w() - fltk3::box_dw(box()));
	tow = wiw;
	tiw = tow - fltk3::box_dw(table->box());
	wih = ( h() - fltk3::box_dh(box()));
	toh = wih;
	tih = toh - fltk3::box_dh(table->box());
...
//
// End of "$Id: Table.cxx 9479 2012-05-10 22:06:06Z matt $".
//
