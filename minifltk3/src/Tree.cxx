//
// "$Id: Tree.cxx 9479 2012-05-10 22:06:06Z matt $"
//
...
static char **parse_path(const char *path)
{
	...
	*sout = 0;
	...
	while ( seps-- > 0 ) {
		if ( *sin ) {
			arr[t++] = sin;        // skips empty fields, e.g. '//'
		}
		sin += (strlen(sin) + 1);
	}
	arr[t] = 0;
...
fltk3::TreeItem *fltk3::Tree::next_visible_item(fltk3::TreeItem *item, unsigned int dir)
{
	...
	switch ( dir ) {
	case fltk3::UpKey:
		return(item->prev_displayed(_prefs));
	case fltk3::DownKey:
		return(item->next_displayed(_prefs));
	default:
		return(item->next_displayed(_prefs));
	}
}
			...
			if ( _item_focus ) {
				...
				case fltk3::DownKey: {	// next item down
					...
					if ( _item_focus ) {					// item in focus?
						...
						if ( itemtop < 0 ) {
							show_item_top(_item_focus);
						}
						if ( itembot > h() ) {
							show_item_bottom(_item_focus);
						}
						// Extend selection
...
//
// End of "$Id: Tree.cxx 9479 2012-05-10 22:06:06Z matt $".
//
