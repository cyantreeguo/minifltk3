//
// "$Id: Counter.cxx 9638 2012-07-24 04:41:38Z matt $"
//
...
void fltk3::Counter::draw()
{
	...
	fltk3::draw_box(boxtype[0], xx[0], 0, ww[0], h(), fltk3::BACKGROUND2_COLOR, Box::TIE_LEFT|Box::TIE_RIGHT);
	...
	if (type() == fltk3::NORMAL_COUNTER) {
		fltk3::draw_box(boxtype[1], xx[1], 0, ww[1], h(), color(), Box::TIE_RIGHT);
		fltk3::draw_symbol("@-4<<", xx[1], 0, ww[1], h(), selcolor);
		fltk3::draw_box(boxtype[2], xx[2], 0, ww[2], h(), color(), Box::TIE_LEFT|Box::TIE_RIGHT);
		fltk3::draw_symbol("@-4<",  xx[2], 0, ww[2], h(), selcolor);
	} else {
		fltk3::draw_box(boxtype[2], xx[2], 0, ww[2], h(), color(), Box::TIE_RIGHT);
		fltk3::draw_symbol("@-4<",  xx[2], 0, ww[2], h(), selcolor);
	}
	if (type() == fltk3::NORMAL_COUNTER) {
		fltk3::draw_box(boxtype[3], xx[3], 0, ww[3], h(), color(), Box::TIE_LEFT|Box::TIE_RIGHT);
		fltk3::draw_symbol("@-4>",  xx[3], 0, ww[3], h(), selcolor);
		fltk3::draw_box(boxtype[4], xx[4], 0, ww[4], h(), color(), Box::TIE_LEFT);
		fltk3::draw_symbol("@-4>>", xx[4], 0, ww[4], h(), selcolor);
	} else {
		fltk3::draw_box(boxtype[3], xx[3], 0, ww[3], h(), color(), Box::TIE_LEFT);
		fltk3::draw_symbol("@-4>",  xx[3], 0, ww[3], h(), selcolor);
	}
}
 ...
 */
fltk3::Counter::Counter(int X, int Y, int W, int H, const char* L) : fltk3::Valuator(X, Y, W, H, L)
{
	box(fltk3::UP_BOX);
...
fltk3::SimpleCounter::SimpleCounter(int x,int y,int w,int h, const char *l)
	: fltk3::Counter(x,y,w,h,l)
{
	type(fltk3::SIMPLE_COUNTER);
}


//
// End of "$Id: Counter.cxx 9638 2012-07-24 04:41:38Z matt $".
//
