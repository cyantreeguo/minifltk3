//
// "$Id: run.cxx 9813 2013-02-05 15:40:17Z manolo $"
//
...
//           window, pushed(), grab(), modal());
	...
	case fltk3::SHORTCUT:
		if (grab()) {
			wi = grab();        // send it to grab window
			break;
		}
...
void fltk3::Window::hide()
{
	...
	int count = 0;
	fltk3::Window *win, **doit = NULL;
	for (win = fltk3::first_window(); win && i; win = fltk3::next_window(win)) {
		if (win->non_modal() && GetWindow(fl_xid(win), GW_OWNER) == i->xid) {
			count++;
		}
	}
	if (count) {
		doit = new fltk3::Window*[count];
		count = 0;
		for (win = fltk3::first_window(); win && i; win = fltk3::next_window(win)) {
			if (win->non_modal() && GetWindow(fl_xid(win), GW_OWNER) == i->xid) {
				doit[count++] = win;
...
/** Back-compatibility only: The single-argument call can be used to
    ...
    of this.</i>
*/
void fltk3::selection_owner(fltk3::Widget *owner)
{
	...
	if (window()) {
		...
		if (align() && !(align() & fltk3::ALIGN_INSIDE) && window()->shown()) {
			...
			case fltk3::ALIGN_TOP_LEFT:
				window()->damage(fltk3::DAMAGE_EXPOSE, x(), y()-H, W, H);
				break;
			case fltk3::ALIGN_TOP:
				window()->damage(fltk3::DAMAGE_EXPOSE, x()+(w()-W)/2, y()-H, W, H);
				break;
			case fltk3::ALIGN_TOP_RIGHT:
				window()->damage(fltk3::DAMAGE_EXPOSE, x()+w()-W, y()-H, W, H);
				break;
			case fltk3::ALIGN_LEFT_TOP:
				window()->damage(fltk3::DAMAGE_EXPOSE, x()-W, y(), W, H);
				break;
			case fltk3::ALIGN_RIGHT_TOP:
				window()->damage(fltk3::DAMAGE_EXPOSE, x()+w(), y(), W, H);
				break;
			case fltk3::ALIGN_LEFT:
				window()->damage(fltk3::DAMAGE_EXPOSE, x()-W, y()+(h()-H)/2, W, H);
				break;
			case fltk3::ALIGN_RIGHT:
				window()->damage(fltk3::DAMAGE_EXPOSE, x()+w(), y()+(h()-H)/2, W, H);
				break;
			case fltk3::ALIGN_LEFT_BOTTOM:
				window()->damage(fltk3::DAMAGE_EXPOSE, x()-W, y()+h()-H, W, H);
				break;
			...
			case fltk3::ALIGN_BOTTOM_RIGHT:
				...
				break;
			default:
				window()->damage(fltk3::DAMAGE_ALL);
				break;
			}
		} else {
...
//
// End of "$Id: run.cxx 9813 2013-02-05 15:40:17Z manolo $".
//
