//
// "$Id: x11_set_fonts.cxx 9011 2011-08-25 21:50:59Z matt $"
//
	...
	if (!f->fontname[0]) {
		...
		if (*p != '-') { // non-standard font, just replace * with spaces:
			...
			*o = 0;
			...
			// get the family:
			const char *x = fltk3::font_word(p,2);
			if (*x) x++;
			if (*x=='*') x++;
			if (!*x) {
				if (ap) *ap = 0;
	...
	static int ultrasort(const void *aa, const void *bb)
	{
		...
		for (int n = 2; n <= 6; n++) {
			...
			if (at < 0) {
				...
				if (bt < 0) return -1;
				a = fltk3::font_word(a,1);
				if (*a) a++;
				b = fltk3::font_word(b,1);
				if (*b) b++;
				atype |= at;
				btype |= bt;
			}
		...
		a = fltk3::font_word(a,6);
		if (*a) a++;
		b = fltk3::font_word(b,6);
		if (*b) b++;
		if (use_registry(a)) {
...
//
// End of "$Id: x11_set_fonts.cxx 9011 2011-08-25 21:50:59Z matt $".
//
