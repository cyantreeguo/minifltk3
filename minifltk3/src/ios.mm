//
// "$Id: cocoa.mm 9969 2013-09-06 09:29:36Z manolo $"
//
...
void DataReady::AddFD(int n, int events, void (*cb)(int, void *), void *v) {
	...
	if (i >= fd_array_size) {
		...
		if (!fds) {temp = (FD *)malloc(fd_array_size * sizeof(FD)); } else {temp = (FD *)realloc(fds, fd_array_size * sizeof(FD)); }
		if (!temp) return;
		fds = temp;
	}
	fds[i].cb  = cb;
...
void* DataReady::DataReadyThread(void *o) {
	...
	while (1) {                 // loop until thread cancel or error
		...
		switch (ret) {
			...
			default:  // DATA READY
				{
					if (FD_ISSET(cancelpipe, &r) || FD_ISSET(cancelpipe, &x)) {return (NULL); }                     // just exit   // cancel?
...
static void breakMacEventLoop() {
	...
	NSPoint pt = { 0, 0 };
	NSEvent *event = [NSEvent otherEventWithType: NSApplicationDefined location: pt
								   modifierFlags: 0 timestamp: 0 windowNumber: 0 context: NULL subtype: FLTKTimerEvent data1: 0 data2: 0];
	[NSApp postEvent: event atStart: NO];
	fl_unlock_function();
}
...
static void do_timer(CFRunLoopTimerRef timer, void *data) {
	...
	if (current_timer && current_timer->pending == 0) delete_timer(*current_timer);
	current_timer = NULL;
	breakMacEventLoop();
}
...
static void update_e_xy_and_e_xy_root(NSWindow *nsw, NSEvent *theEvent = 0) {
	NSPoint pt;
	if (theEvent && [theEvent window] == nsw) pt = [theEvent locationInWindow];
	else pt = [nsw mouseLocationOutsideOfEventStream];
	fltk3::e_x = int(pt.x);
...
static void cocoaMouseHandler(NSEvent *theEvent) {
	...
	switch (etype) {
		...
		case NSOtherMouseDown:
			...
			if ([theEvent clickCount] > 1) fltk3::e_clicks++;
			else fltk3::e_clicks = 0;
			// fall through
		case NSLeftMouseUp:
		case NSRightMouseUp:
		case NSOtherMouseUp:
		...
		case NSOtherMouseDragged:
			{
				...
				if (!sendEvent) {
					...
					if (fabs(pos.x - px) > 5 || fabs(pos.y - py) > 5) fltk3::e_is_click = 0;
				}
...
- (void)windowDidResignKey: (NSNotification *)notif {
	...
	if (window->fullscreen_active()) [nsw setLevel: NSNormalWindowLevel];
...
- (void)windowDidBecomeKey: (NSNotification *)notif {
	...
	if (w->fullscreen_active()) [nsw setLevel: NSStatusWindowLevel];
...
- (NSApplicationTerminateReply)applicationShouldTerminate: (UIApplication *)sender;
- (void)applicationDidBecomeActive: (NSNotification *)notify;
- (void)applicationDidChangeScreenParameters: (NSNotification *)aNotification;
- (void)applicationWillResignActive: (NSNotification *)notify;
- (void)applicationWillHide: (NSNotification *)notify;
- (void)applicationWillUnhide: (NSNotification *)notify;
- (BOOL)application: (UIApplication *)theApplication openFile: (NSString *)filename;
	...
	for (x = Fl_X::first; x; x = x->next) {
		...
		if (win && cw && [cw isVisible]) {
			if (win->modal()) {
				...
				if (topModal) [cw orderWindow: NSWindowBelow relativeTo: [topModal windowNumber]];
				else topModal = cw;
			} else if (win->non_modal()) {
				[cw setLevel: NSFloatingWindowLevel];
				if (topNonModal) [cw orderWindow: NSWindowBelow relativeTo: [topNonModal windowNumber]];
				else topNonModal = cw;
			} else {
				if (top)
				;
...
- (void)applicationWillResignActive: (NSNotification *)notify {
	...
	for (x = Fl_X::first; x; x = x->next) {
		...
		fltk3::Window *win = x->w;
		if (win && cw) {
			if (win->modal()) {} else if (win->non_modal()) {} else {
				if (!top) top = cw;
			}
		}
	}
...
void fl_open_display() 
{
	...
	if (!beenHereDoneThat) {
		...
		if (need_new_nsapp) [UIApplication sharedApplication];
		...
		if (!GetCurrentProcess(&cur_psn) && !GetFrontProcess(&front_psn) && !SameProcess(&front_psn, &cur_psn, &same_psn) && !same_psn) {
...
- (void)flagsChanged: (NSEvent *)theEvent {
	...
	if (tMods) {
		...
		if (fltk3::e_keysym) sendEvent = (prevMods < mods) ? fltk3::KEYBOARD : fltk3::KEYUP;
		fltk3::e_length = 0;
		fltk3::e_text = (char *)"";
		prevMods = mods;
	}
...
void fltk3::Window::size_range_() {
	...
	get_window_frame_sizes(bx, by, bt);
	size_range_set = 1;
	NSSize minSize = { (CGFloat)(minw), (CGFloat)(minh + bt) };
	NSSize maxSize = { (CGFloat)(maxw ? maxw : 32000), (CGFloat)(maxh ? maxh + bt : 32000) };
	if (i && i->xid) {
...
static void allocatePasteboard() {
	if (!myPasteboard) PasteboardCreate(kPasteboardClipboard, &myPasteboard);
}
...
void Fl_X::set_cursor(fltk3::Cursor c) {
	NSCursor *icrsr;
	switch (c) {
		case fltk3::CURSOR_CROSS:
			icrsr = [NSCursor crosshairCursor]; break;
		case fltk3::CURSOR_WAIT:
			static NSCursor *watch = nil;
			watch = PrepareCursor(watch,  &Fl_X::watch_cursor_image);
			icrsr = watch;
			break;
		case fltk3::CURSOR_INSERT:
			icrsr = [NSCursor IBeamCursor]; break;
		case fltk3::CURSOR_N:
			icrsr = [NSCursor resizeUpCursor]; break;
		case fltk3::CURSOR_S:
			icrsr = [NSCursor resizeDownCursor]; break;
		case fltk3::CURSOR_NS:
			icrsr = [NSCursor resizeUpDownCursor]; break;
		...
		case fltk3::CURSOR_HAND:
			icrsr = [NSCursor pointingHandCursor]; break;
		case fltk3::CURSOR_MOVE:
			icrsr = [NSCursor openHandCursor]; break;
		...
		case fltk3::CURSOR_NESW:
			...
			break;
		case fltk3::CURSOR_E:
			icrsr = [NSCursor resizeRightCursor]; break;
		case fltk3::CURSOR_W:
			icrsr = [NSCursor resizeLeftCursor]; break;
		case fltk3::CURSOR_WE:
			icrsr = [NSCursor resizeLeftRightCursor]; break;
...
*/ {
	...
	unsigned char *pdata = (unsigned char *)CGBitmapContextGetData(c);
	fltk3::Image *img = new fltk3::ImageRGB(pdata, CGBitmapContextGetWidth(c), CGBitmapContextGetHeight(c), 4);
...
void fltk3::PagedDevice::print_window(fltk3::Window *win, int x_offset, int y_offset) {
	...
	// and print it
	CGRect rect = { { (CGFloat)(x_offset), (CGFloat)(y_offset) }, { (CGFloat)(win->w()), (CGFloat)(bt) } };
...
//
// End of "$Id: cocoa.mm 9969 2013-09-06 09:29:36Z manolo $".
//
