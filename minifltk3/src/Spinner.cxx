//
// "$Id: Spinner.cxx 9650 2012-08-02 19:47:06Z matt $"
//
...
void fltk3::Spinner::UpButton::draw()
{
	...
	Box *b = value() ? box()->downbox() : box();
	Color c = value() ? selection_color() : color();
	//fltk3::Button::draw(b, 0, 0, w(), h(), c, fltk3::Box::TIE_LEFT|fltk3::Box::TIE_BOTTOM);
	Button::draw(b, 0, 0, w(), h(), c, fltk3::Box::TIE_LEFT|fltk3::Box::TIE_BOTTOM);
...
void fltk3::Spinner::DownButton::draw()
{
	Box *b = value() ? box()->downbox() : box();
	Color c = value() ? selection_color() : color();
	//fltk3::Button::draw(b, 0, 0, w(), h(), c, fltk3::Box::TIE_LEFT|fltk3::Box::TIE_TOP);
	Button::draw(b, 0, 0, w(), h(), c, fltk3::Box::TIE_LEFT|fltk3::Box::TIE_TOP);
	fltk3::font(labelfont(), labelsize()/2);
	fltk3::color(fltk3::color_average(color(), fltk3::BLACK, 0.5));
	fltk3::draw("@2>", 0, 0, w(), h()-2, fltk3::ALIGN_INSIDE);
}

fltk3::Spinner::SpInput::SpInput(int x, int y, int w, int h, const char *l) : fltk3::Input(x, y, w, h, l)
{
}
...
void fltk3::Spinner::SpInput::draw()
{
	...
	Input::draw(box(), 0, 0, w(), h(), color(), fltk3::Box::TIE_RIGHT|fltk3::Box::TIE_HIDDEN);
}
...
//
// End of "$Id: Spinner.cxx 9650 2012-08-02 19:47:06Z matt $".
//
