//
// "$Id: Image.cxx 9930 2013-05-31 13:04:25Z manolo $"
//
...
// RGB image class...
//
size_t fltk3::ImageRGB::max_size_ = ~((size_t)0);

/**  The destructor free all memory and server resources that are used by  the image. */
fltk3::ImageRGB::~ImageRGB()
{
	uncache();
	if (alloc_array) delete[] (uchar *)array;
}

void fltk3::ImageRGB::uncache()
{
...
fltk3::Image *fltk3::ImageRGB::copy(int W, int H)
{
	fltk3::ImageRGB	*new_image;	// New RGB image
	...
	if ((W == w() && H == h()) ||
	    !w() || !h() || !d() || !array) {
		if (array) {
			// Make a copy of the image data and return a new fltk3::ImageRGB...
			...
			new_image = new fltk3::ImageRGB(new_array, w(), h(), d());
			new_image->alloc_array = 1;

			return new_image;
		} else return new fltk3::ImageRGB(array, w(), h(), d(), ld());
	}
	if (W <= 0 || H <= 0) return 0;
...
void fltk3::ImageRGB::color_average(fltk3::Color c, float i)
{
...
void fltk3::ImageRGB::desaturate()
{
...
// alpha compositing...
static void alpha_blend(fltk3::ImageRGB *img, int X, int Y, int W, int H, int cx, int cy)
{
	int ld = img->ld();
...
void fltk3::ImageRGB::draw(int XP, int YP, int WP, int HP, int cx, int cy)
{
	fltk3::graphics_driver->draw(this, XP, YP, WP, HP, cx, cy);
}

static int start(fltk3::ImageRGB *img, int XP, int YP, int WP, int HP, int w, int h, int &cx, int &cy,
...
void fltk3::QuartzGraphicsDriver::draw(fltk3::ImageRGB *img, int XP, int YP, int WP, int HP, int cx, int cy)
{
	int X, Y, W, H;
	...
	if (!img->id_) {
		...
		if (is_printer()) {
			...
			fltk3::ImageRGB* img2 = (fltk3::ImageRGB*)img->copy();
			img2->alloc_array = 0;
	...
	if (img->id_ && fl_gc) {
		...
		CGRect rect = { { (CGFloat)(X+origin_x()), (CGFloat)(Y+origin_y()) }, { (CGFloat)(W), (CGFloat)(H) } };
...
#elif defined(WIN32)
void fltk3::GDIGraphicsDriver::draw(fltk3::ImageRGB *img, int XP, int YP, int WP, int HP, int cx, int cy)
{
	int X, Y, W, H;
	// Don't draw an empty image...
	if (!img->d() || !img->array) {
...
#else
void fltk3::XlibGraphicsDriver::draw(fltk3::ImageRGB *img, int XP, int YP, int WP, int HP, int cx, int cy)
{
	int X, Y, W, H;
	// Don't draw an empty image...
	if (!img->d() || !img->array) {
...
void fltk3::ImageRGB::label(fltk3::Widget* widget)
{
	widget->image(this);
}

void fltk3::ImageRGB::label(fltk3::MenuItem* m)
{
	m->label(fltk3::IMAGE_LABEL, (const char*)this);
}
...
//
// End of "$Id: Image.cxx 9930 2013-05-31 13:04:25Z manolo $".
//
