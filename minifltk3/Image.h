//
// "$Id: Image.h 9710 2012-11-09 16:29:43Z manolo $"
//
...
/* \file
 fltk3::Image, fltk3::ImageRGB classes . */
...
public:
	...
	Image(const uchar*, int W, int H, int D=3, int =0) {
		w_ = W;
		h_ = H;
		d_ = D;
		ld_ = 0;
	...
	void draw(int X, int Y) {
		draw(X, Y, w(), h(), 0, 0);       // platform dependent
	}
	virtual void uncache();
};

/**
 The fltk3::ImageRGB class supports caching and drawing
 ...
 <P>fltk3::ImageRGB is defined in
 &lt;fltk3/Image.h&gt;, however for compatibility reasons
 &lt;fltk3/ImageRGB.h&gt; should be included.
 */
class FLTK3_EXPORT ImageRGB : public Image
{
...
public:
	...
	ImageRGB(const uchar *bits, int W, int H, int D=3, int LD=0) :
		fltk3::Image(W,H,D), array(bits), alloc_array(0), id_(0), mask_(0) {
	...
	virtual void label(fltk3::MenuItem*m);
	virtual void uncache();
	/** Sets the maximum allowed image size in bytes when creating an ImageRGB object.

	 The image size in bytes of an ImageRGB object is the value of the product w() * h() * d().
	 If this product exceeds size, the created object of a derived class of ImageRGB
	 won't be loaded with the image data.
	 This does not apply to direct RGB image creation with
	 ImageRGB::ImageRGB(const uchar *bits, int W, int H, int D, int LD).
	 The default max_size() value is essentially infinite.
	 */
	static void max_size(size_t size) {
		max_size_ = size;
	}
	/** Returns the maximum allowed image size in bytes when creating an ImageRGB object.

	 \sa  void ImageRGB::max_size(size_t)
	 */
...
//
// End of "$Id: Image.h 9710 2012-11-09 16:29:43Z manolo $".
//
