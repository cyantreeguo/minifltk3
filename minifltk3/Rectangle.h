//
// "$Id: Rectangle.h 9648 2012-07-28 13:38:18Z matt $"
//
...
public:
	...
	void merge(const fltk3::Rectangle& R) {
		if (R.empty()) return;
		if (empty()) {
			*this = R;
			return;
	...
	void intersect(const fltk3::Rectangle& R) {
		if (R.x() > x()) set_x(R.x());
		if (R.r() < r()) set_r(R.r());
		if (R.y() > y()) set_y(R.y());
		if (R.b() < b()) set_b(R.b());
	...
	bool intersects(const fltk3::Rectangle& R) {
