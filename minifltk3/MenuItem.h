//
// "$Id: MenuItem.h 9316 2012-04-02 15:46:03Z manolo $"
//
...
struct FLTK3_EXPORT MenuItem {
	...
	const fltk3::MenuItem *first() const {
		return next(0);
	}

	/** Returns the first menu item, same as next(0). */
	fltk3::MenuItem *first() {
		return next(0);
	}
	...
	fltk3::Labeltype labeltype() const {
		return (fltk3::Labeltype)labeltype_;
	}
	...
	/**
	 ...
	 The value fltk3::NORMAL_LABEL prints the label as text.
	 */
	void labeltype(fltk3::Labeltype a) {
		labeltype_ = a;
	}
	...
	void labelsize(fltk3::Fontsize a) {
		labelsize_ = a;
	}
	 ...
	 */
	fltk3::CallbackPtr callback() const {
		return callback_;
	}
	 ...
	 */
	void callback(fltk3::Callback* c, void* p) {
		callback_=c;
		user_data_=p;
	}
	...
	/**
	 ...
	 \see fltk3::CallbackPtr fltk3::MenuItem::callback() const
	 */
	void callback(fltk3::Callback* c) {
		callback_=c;
	}
	...
	/**
	 ...
	 \see fltk3::CallbackPtr fltk3::MenuItem::callback() const
	 */
	void callback(fltk3::Callback0*c) {
		callback_=(fltk3::Callback*)c;
	}
	...
	/**
	 ...
	 \see fltk3::CallbackPtr fltk3::MenuItem::callback() const
	 */
	void callback(fltk3::Callback1*c, long p=0) {
		callback_=(fltk3::Callback*)c;
		user_data_=(void*)p;
	}
	 ...
	 */
	int submenu() const {
		return flags&(SUBMENU|SUBMENU_POINTER);
	}
	/**
	 ...
	 */
	int radio() const {
		return flags&MENU_RADIO;
	}
	/** Returns the current value of the check or radio item. */
	int value() const {
		return flags&MENU_VALUE;
	}
	/**
	...
	int visible() const {
		return !(flags&MENU_INVISIBLE);
	}

	/** Makes an item visible in the menu. */
	void show() {
		flags &= ~MENU_INVISIBLE;
	}
	...
	int activevisible() const {
		return !(flags & (MENU_INACTIVE|MENU_INVISIBLE));
	}

	// compatibility for FLUID so it can set the image of a menu item...

	/** compatibility api for FLUID, same as a->label(this) */
	void image(fltk3::Image* a) {
		a->label(this);
	}
	...
	/**
	 ...
	 You must first check that callback() is non-zero before calling this.
	 */
	void do_callback(fltk3::Widget* o,void* arg) const {
		callback_(o, arg);
	}
	...
	/**
	 ...
	 You must first check that callback() is non-zero before calling this.
	 */
	void do_callback(fltk3::Widget* o,long arg) const {
...
//
// End of "$Id: MenuItem.h 9316 2012-04-02 15:46:03Z manolo $".
//
